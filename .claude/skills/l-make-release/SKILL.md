...
3. **Write the six bilingual changelog source pages.** Create or update exactly these pages for
   ...
   Never edit `packages/*/CHANGELOG.md` by hand. The pre-commit formatter hook is likewise kept off
   those files by the root `.mdx-formatter-ignore` guard passed via `--ignore-path`;
   `.mdx-formatter.json`'s `exclude` alone does not cover literal file paths, so a release operator
   who sees `format-mdx` run over a staged changelog batch knows the generated bytes are unmodified.
