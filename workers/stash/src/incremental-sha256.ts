...
/** Small Workers-native incremental SHA-256 used so R2-bound bodies are never buffered whole. */
export class IncrementalSha256 {
  ...
  digest(): string {
    ...
    view.setUint32(56, bitHigh);
    view.setUint32(60, bitLow);
    this.#compress(this.#block);
  ...
  #compress(block: Uint8Array): void {
    ...
    for (let index = 0; index < 64; index++) {
      ...
      const first = (h! + sum1 + choice + K[index]! + words[index]!) >>> 0;
