...
パッケージのルート共用体に含まれるのは、正確に `home`、`stash`、`file`、`diff`、`edit`、`change set`、
`new-file`、`tokens` です。ホストは、これらのルート値を独自のルーターへ割り当てられます。ホストされた
Viewer には提案一覧の URL がありますが、パッケージレベルの `change sets` ルート種別はありません。
