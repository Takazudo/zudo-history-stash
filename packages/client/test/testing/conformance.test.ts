  ...
  it("passes the complete data-driven sequence against the fake", async () => {
    ...
    expect(CONFORMANCE_TRACE.map((step) => step.name)).toEqual(
      expect.arrayContaining([
        ...
        "multi-entry commit creates atomically",
        "commit replay preserves the atomic result",
        "commit conflict exposes per-entry conflicts",
        ...
        "stale change set can be rejected",
        ...
        "snapshot resolves the approved commit under a prefix",
        "prefix fence creates an atomic multi-entry commit",
        "unrelated prefix advances the stash cursor",
