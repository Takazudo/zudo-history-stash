  ...
  it("passes the complete data-driven sequence against the fake", async () => {
    ...
    expect(CONFORMANCE_TRACE.map((step) => step.name)).toEqual(
      expect.arrayContaining([
        ...
        "create proposal against the exact head",
        "proposal create replays the same record",
        "proposal list defaults to open and preserves totals",
        "proposal detail returns the immutable candidate body",
        "proposal diff is base to candidate before approval",
        "proposal approval appends a normal audited version",
        "proposal reapproval reconstructs the first winner",
        "approved proposal is ordinary stamped file history",
        "applied proposal diff stays immutable while current moves",
        "create proposal on the former head for stale refusal",
        "stale proposal approval refuses without appending",
        "stale open proposal can be rejected",
        "proposal rerejection is idempotent",
